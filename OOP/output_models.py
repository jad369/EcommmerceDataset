...
class CustomerSummaryOutputModel:
    ...
    def to_dict(self):
        return {"total_customers": self.total_customers, "customers": self.customers}
...
class InvoiceSummaryOutputModel:
    ...
    def __init__(self, invoices_dict):
        ...
        total_revenue = sum(
            invoice.get_invoice_total() for invoice in invoices_dict.values()
        )
        self.total_revenue = round(total_revenue, 2)
        self.invoices = [
            InvoiceOutputModel(invoice).to_dict() for invoice in invoices_dict.values()
        ]
...
class ProductCatalogOutputModel:
    ...
    def __init__(self, products_dict):
        ...
        self.products = [
            ProductOutputModel(product).to_dict() for product in products_dict.values()
        ]

    def to_dict(self):
        return {"total_products": self.total_products, "products": self.products}
